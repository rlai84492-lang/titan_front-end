...
export function DashboardProvider({ children }) {
  ...
  // ← Track karta hai abhi kis flow+range ka data screen pe hai,
  //   taaki auto-refresh (30s) pe loader na dikhe — sirf flow/range
  //   badalne pe hi dikhe
  ...
  const loadDashboard = useCallback(async (flow = 'bday_t10', opts = {}) => {
    ...
    // ← Loader SIRF tab dikhao jab flow/range badla ho (fresh load).
    //    Same flow+range pe auto-refresh (30s) silent rahega.
    const isFreshLoad =
      currentFlowRef.current === null ||
      currentFlowRef.current.flow  !== flow  ||
      currentFlowRef.current.range !== range ||
      currentFlowRef.current.start !== start ||
      currentFlowRef.current.end   !== end
    ...
    try {
      ...
      // ← Ab ye flow+range "current" ban gaya — agli baar isi ke liye loader nahi dikhega
      currentFlowRef.current = { flow, range, start, end }
    } catch (err) {
      console.error('%c[Dashboard] API call failed ❌', 'color:#EF4444;font-weight:bold', err)
      setError(err.message)
    } finally {

...
// const res = await fetch(`${API_BASE}/api/dashboard/counts`);
...
// ── Dashboard — flow + date range dono support karta hai ─────────
export const fetchDashboard = async (flow = 'bday_t10', dateOpts = {}) => {
  ...
  if (dateRange) {
    params.append('range', dateRange) // today | 7days | 30days | custom
  }
  if (dateRange === 'custom' && customStart && customEnd) {
    params.append('startDate', customStart) // yyyy-MM-dd
    params.append('endDate', customEnd)
  }

  return authFetch(`/api/dashboard?${params.toString()}`)
}

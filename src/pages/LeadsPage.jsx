...
//       console.log(res , "hgcvnbm")
//       if (!mountedRef.current) return
//       setLeads(Array.isArray(res.leads) ? res.leads : [])
//       setTotalLeads(res.leadMetrics.total  ?? 0)
...
//     { label: 'Callback', value: cbDiscovery, accent: 'purple', icon: '📞' },
...
export default function LeadsPage() {
  ...
  const [leads,        setLeads]        = useState([])
  const [page,         setPage]         = useState(0)
  const [totalLeads,   setTotalLeads]   = useState(0)   // paginated row count (for pagination math)
  const [totalPages,   setTotalPages]   = useState(1)
  const [pageLoading,  setPageLoading]  = useState(false)

  // ★ NAYA — poore (cross-page) totals server se. Cards isi se banenge.
  const [leadMetrics,  setLeadMetrics]  = useState({
    total: 0, new_count: 0, callback: 0, store_visit: 0, converted: 0,
  })
  ...
  const loadPage = useCallback(async (targetPage) => {
    ...
    try {
      ...
      if (!mountedRef.current) return
      setLeads(Array.isArray(res.leads) ? res.leads : [])
      setLeadMetrics(res.leadMetrics ?? {})     // ★ cross-page totals (total / callback / store_visit / new_count / converted)
      setTotalLeads(res.totalLeads ?? 0)        // ★ pagination ke liye actual row count
      setTotalPages(res.totalPages ?? 1)
      setPage(res.currentPage ?? targetPage)
    } catch (err) {
      console.error('[LeadsPage] fetchLeads failed', err)
  ...
  // ── Metrics — SAB leadMetrics (poore server totals) se, current page se NAHI ──
  const metrics = [
    { label: 'Total Leads',  value: leadMetrics.total       ?? 0, accent: 'blue',   icon: '🎯' },
    { label: 'New Leads',    value: leadMetrics.new_count   ?? 0, accent: 'orange', icon: '🆕' },
    { label: 'Callback',     value: leadMetrics.callback    ?? 0, accent: 'purple', icon: '📞' },
    { label: 'Store Visits', value: leadMetrics.store_visit ?? 0, accent: 'green',  icon: '🏪' },
  ]

  const totalForDisplay = leadMetrics.total ?? 0   // header/title ke liye poora total
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-xl px-5 py-3 flex items-center justify-between flex-wrap gap-3"
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-bold text-sm" style={{ color: cfg.hex }}>{cfg.label}</div>
            <div className="text-[10px] text-gray-400">
              Lead Management · {totalForDisplay.toLocaleString('en-IN')} total leads
            </div>
          </div>
        </div>
        <ExportButtons leads={leads} activeFlow={activeFlow} />
      ...
      {/* Leads Table — server-paginated */}
      <CardOne
        title={`Leads (${totalForDisplay.toLocaleString('en-IN')} total)`}
        subtitle={`${cfg.emoji} ${cfg.label} · Showing page ${page + 1} of ${totalPages}`}
        icon="🎯"
      >
        {/* Page loading shimmer — shown on subsequent page changes (not first load) */}

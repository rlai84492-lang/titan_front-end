...
// Explicitly batata hai ki SAARE leads (jitne bhi hon — 2000, 30000)
// export honge, chahe pagination kuch bhi ho. Alag se Excel/PDF
// choose karne ka dropdown deta hai, apna loading state rakhta hai.
// ════════════════════════════════════════════════════════════
function ExportAllButton({ leads, activeFlow }) {
  const cfg = FLOW_CONFIG[activeFlow] || FLOW_CONFIG.all
  const [menuOpen, setMenuOpen]   = useState(false)
  const [exporting, setExporting] = useState(false)   // ← konsa export chal raha hai: false | 'excel' | 'pdf'
  ...
  const runExport = async (type) => {
    ...
    try {
      // setTimeout se UI ko ek frame milta hai spinner render karne ke liye,
      // warna heavy export (30000 rows) synchronous chalne se UI freeze
      // ho jaata aur spinner kabhi dikhta hi nahi
      await new Promise(resolve => setTimeout(resolve, 50))
      ...
      {menuOpen && !exporting && (
        <div className="absolute right-0 mt-1.5 w-56 rounded-xl border border-[#EEEBE6] bg-white shadow-lg overflow-hidden z-20">
          <div className="px-3 py-2 text-[10px] font-semibold text-[#B0A9A1] border-b border-[#F4F1ED]">
            All {leads?.length ?? 0} leads will be exported
          </div>
          <button
...
// ── Existing Export Buttons — same as before, untouched ─────────
function ExportButtons({ leads, activeFlow }) {
  const cfg = FLOW_CONFIG[activeFlow] || FLOW_CONFIG.all
